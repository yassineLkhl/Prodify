    ...
    public ResponseEntity<PaymentResponse> createCheckoutSession(
        ...
        if (order.getStatus() != OrderStatus.PENDING) {
            throw new RuntimeException(
                    "Cette commande ne peut plus être payée. Statut actuel: " + order.getStatus());
        }
        ...
        try {
            ...
            PaymentResponse response = PaymentResponse.builder().url(session.getUrl()).build();
            ...
            e.printStackTrace();
            System.err.println(
                    "Erreur inattendue lors de la création de la session de paiement: "
                            + e.getMessage());
