...
/**
 * Classe utilitaire pour construire les Specifications JPA
 * basées sur les critères de recherche.
 */
public class TrackSpecification {
    ...
    /**
     * Construit une Specification dynamique à partir des critères de recherche.
     * Chaque critère présent (non null) génère un prédicat WHERE.
     *
    ...
    public static Specification<Track> getSpecifications(TrackSearchCriteria criteria) {
        return (root, query, criteriaBuilder) -> {
            ...
            if (Objects.nonNull(criteria.getMinBpm())) {
                predicates.add(
                        criteriaBuilder.greaterThanOrEqualTo(root.get("bpm"), criteria.getMinBpm())
                );
            }
